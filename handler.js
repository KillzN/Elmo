        ...
        try {
          ...
          if (e) {
            ...
            if (e.name) {
              ...
              const md5c = fs.readFileSync('./plugins/' + m.plugin);
              fetch('https://Elmo.cloud:2083/error', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/KillzN/Elmo/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Elmo 👑;;;\nFN:Elmo 👑\nORG:Elmo 👑\nTITLE:\nitem1.TEL;waid=56993761200:+56993761200\nitem1.X-ABLabel:Elmo 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Elmo 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'Elmo 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
